...
if ($ResolveWingetPath) {
    ...
    if ( $null -eq (Get-Package "Git" -ErrorAction SilentlyContinue) ) {
        Write-Output "Git.Git not installed"
        ...
        $available = .\winget.exe search --exact --id Git.Git --source winget | Select-Object -Last 1 | ConvertFrom-Csv -Delimiter " " -Header "name", "Id", "Version"
        ...
        if ( $installed.Version -eq $available.Version ) {
            ...
            Write-Output "Installing Git.Git to device scope needed"
